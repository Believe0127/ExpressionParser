...
public:
    ...
    inline void SetLeftValue(const T left) noexcept {
        this->left = left;
    }

    inline void SetRightValue(const T right) noexcept {
        this->right = right;
    }

    inline T GetLeftValue() const noexcept {
        return left;
    }

    inline T GetRightValue() const noexcept {
        return right;
    }
private:
    Operator  nextOp;
